...
kk.Pixel.valueOf = function (value, baseOf, defalutValue) {
    var p = new kk.Pixel();
    p.set(value);
    return p.valueOf(baseOf, defalutValue);
};
...
kk.Edge.prototype = kk.extend(Object.prototype, {
    ...
    toString: function () {
        return [this.top.toString(), this.right.toString(), this.bottom.toString(), this.left.toString()].join(' ');
    }
});
...
kk.ViewElement.FixLayoutWidth = function (p, inSize, width) {
...
kk.ViewElement.FixLayoutHeight = function (p, inSize, height) {
    ...
    while (p) {
        ...
        if (p instanceof kk.ViewElement) {
            ...
            if (width != kk.Pixel.AUTO) {
                width = kk.ViewElement.FixLayoutWidth(p, inSize, width);
            }

            if (height != kk.Pixel.AUTO) {
                height = kk.ViewElement.FixLayoutHeight(p, inSize, height);
            }
            ...
            if (width == kk.Pixel.AUTO) {
                width = kk.ViewElement.FixLayoutWidth(p, inSize, p.contentSize.width);
                p.frame.width = width;
            }

            if (height == kk.Pixel.AUTO) {
                height = kk.ViewElement.FixLayoutHeight(p, inSize, p.contentSize.height);
                p.frame.height = height;
            }
...
kk.ViewElement.FlexLayout = function (element, isAutoWarp) {
    ...
    while (p) {
        ...
        if (p instanceof kk.ViewElement) {
            ...
            min = p.minHeight.valueOf(inSize.height, 0);
            max = p.maxHeight.valueOf(inSize.height, kk.Pixel.AUTO);
            if (height < min) {
                height = min;
            }
            if (height > max) {
...
kk.ViewElement.prototype = kk.extend(kk.Element.prototype, {
    ...
    viewChangedKey: function (view, key, value) {
        switch (key) {
            ...
            case "animation":
                ...
                break;
            case "z-index":
                view.style.zIndex = parseInt(value);
                break;
        }
    },
        ...
        if (vs.length > 0 && vs[0]) {
            ...
            if (image && vs.length > 1) {
                ...
                if (mCapTop == 0) {
                    ...
                    ctx.drawImage(img, 0, mCapTop + capSize, mCapLeft, mHeight - mCapTop - capSize
                    ...
                    ctx.drawImage(img, mCapLeft + capSize, 0, mWidth - mCapLeft - capSize, mCapTop
    ...
    viewDidLayouted: function (view) {
        ...
        view.style.zIndex = parseInt(this.get("z-index")) || 0;
        this.setNeedUpdateBackgroundView();
    },
    ...
    viewWillDisappear: function (view) {

    },

    viewDidDisappear: function (view) {
    ...
    isHidden: function () {
        return kk.booleanValue(this.get("hidden"));
    },
    ...
    didLayouted: function () {
        if (this.prevSibling) {
            this.zIndex = this.prevSibling.zIndex + 1;
        }
        if (this.view) {
    ...
    onResize: function () {
        this.changedKeys(["border-width", "border-radius"]);
        var e = this.firstChild;
        while (e) {
            if (e instanceof kk.ViewElement) {
                e.onResize();
