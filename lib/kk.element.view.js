    ...
    parse: function (value, view, lineSpacing) {
        if (typeof value == 'string') {
            ...
            for (var i = 0; i < vs.length; i++) {
                ...
                if (kk.Pixel.is(v)) {
                    view.style.fontSize = kk.Pixel.valueOf(v, 0, 0) + 'px';
                    view.style.lineHeight = (kk.Pixel.valueOf(v, 0, 0)  + (lineSpacing || 0)) + 'px';
                } else if (v == 'bold') {
...
kk.ViewElement.FlexLayout = function (element, isAutoWarp) {
    ...
    while (p) {
        ...
        if (p instanceof kk.ViewElement) {
            ...
            if (left + padding.right + margin.right > maxWidth) {
                maxWidth = left + padding.right + margin.right;
            }
...
kk.ViewElement.prototype = kk.extend(kk.Element.prototype, {
    ...
    obtainView: function (view, addSubview) {
        ...
        if (this.view && this.parentElement == view) {
