...
kk.Application = function () {
    ...
    this.data = new kk.Data({
        device: {
            mobile: /[Mm]obile/i.test(navigator.userAgent),
            iPhone: /iPhone/i.test(navigator.userAgent),
            iPad: /iPad/i.test(navigator.userAgent),
            iPod: /iPod/i.test(navigator.userAgent),
            android: /[Aa]ndroid/i.test(navigator.userAgent),
            ios: /(iPhone)|(iPad)|(iPod)/i.test(navigator.userAgent),
            wx: /[MicroMessage]/i.test(navigator.userAgent),
            chrome: /[Chrome]/i.test(navigator.userAgent),
            firefox: /[Firefox]/i.test(navigator.userAgent),
            ie: /[Internet Explorer]/i.test(navigator.userAgent),
...
kk.Application.prototype = kk.extend(kk.EventEmitter.prototype, {
    ...
    run: function (url, view, key, oninfo) {
        ...
        this.window = view ? view : document.body;

        this.view = document.createElement("div");;

        this.view.className = "kk-view kk-app";
        ...
        this.window.appendChild(this.view);
    ...
    open: function (appInfo, url, key) {
        ...
        for (var i = 0; i < kk.openlibs.length; i++) {
            var fn = kk.openlibs[i];
            fn(this, appInfo, url, key);
        }
    ...
    onAction: function (action) {
        ...
        if (action.path) {
            ...
            if (action.type == 'window') {
                ...
                page.element.set("z-index", '100');
                this.openWindow(page);
            } else {
                page.open(action.path);
                page.element.set("z-index", '0');
                this.openPage(page);
            }
        } else if (action.url) {
    ...
    resize: function (width, height) {
        ...
        if (this._barView) {
            height = height - this._barView.clientHeight;
    ...
    clearBarView: function (barView) {
        if (this._barView == barView) {
            this.setBarView(undefined, 0);
        }
    },

    setBarView: function (barView, barHeight) {
        ...
        if (this._barView) {
            var p = this._barView.parentNode;
            if (p) {
                p.removeChild(this._barView);
            }
            delete this._barView;
        }

        var width = this.window.clientWidth;
        var height = this.window.clientHeight - barHeight;
        ...
        if (barView) {
            ...
            var p = barView.parentNode;
            if (p) {
                p.removeChild(barView);
            }
            barView.style.top = 'auto';
            barView.style.bottom = '0px';
            barView.style.width = '100%';
            barView.style.height = barHeight + 'px';
            barView.style.position = 'absolute';
            this.window.appendChild(barView);
        }
    },
